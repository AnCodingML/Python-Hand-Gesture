...
while(cap.isOpened()):
    ...
    #cv2.rectangle(img,(600,600),(400,400),(0,255,0),0)
    ...
    _, threshl = cv2.threshold(blurred, 127, 255,
                               cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    cv2.imshow('Thresholded', threshl)

    (version,_,_) = cv2.__version__.split('.')
    if version == '4':
        contours, hierarchy = cv2.findContours(threshl.copy(),cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    elif version == '3':
        image, contours, hierarchy = cv2.findContours(thresh1.coy(),\
                                cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    elif version == '2':
        contours, hierarchy = cv2.findContours(thresh1.coy(),\
                                cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    ...
    count_defects =0
    cv2.drawContours(threshl,contours, -1,(0,255,0),3)
